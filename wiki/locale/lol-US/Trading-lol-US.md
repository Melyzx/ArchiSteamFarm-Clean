...
ASF INCLUDEZ SUPPORT 4 STEAM NON-INTERACTIV (OFFLINE) TRADEZ. BOTH RECEIVIN (ACCEPTIN/DECLININ) AS WELL AS SENDIN TRADEZ IZ AVAILABLE RITE AWAY AN DOESNT REQUIRE SPESHUL CONFIGURASHUN, BUT OBVIOUSLY REQUIREZ UNRESTRICTD STEAM AKOWNT (TEH WAN DAT SPENT 5$ IN DA STORE ALREADY). ONLY LIMITD TRADIN FUNCSHUNALITY IZ AVAILABLE 4 RESTRICTD ACCOUNTS.
...
ASF WILL ACCEPT ALL `loot`-LIEK TRADEZ BEAN SENT ACROS BOTS, UNLES `DontAcceptBotTrades` IZ SPECIFID IN `TradingPreferences`. IN SHORT, DEFAULT `TradingPreferences` OV `None` WILL CAUSE ASF 2 AUTOMATICALLY ACCEPT TRADEZ FRUM USR WIF `Master` ACCES 2 TEH BOT (EXPLAIND ABOOV), AS WELL AS ALL DONASHUN TRADEZ FRUM OTHR BOTS DAT R TAKIN PART IN ASF PROCES.

FURST OV ALL, IZ POSIBLE 2 DISABLE **ALL** INCOMIN TRADE OFFERS, BY USIN `DisableIncomingTradesParsing` FLAG IN `BotBehaviour`. USIN DAT, AS TEH NAYM IMPLIEZ, WILL DISABLE ALL FUNCSHUNALITY RELATD 2 INCOMIN TRADEZ PARSIN, WHICH INCLUDEZ ABOOV LOGIC, AS WELL AS ALL EXTRA FEATUREZ AVAILABLE BELOW WHICH DEPEND ON REACTIN 2 TEH INCOMIN TRADE OFFR. SINCE DEFAULT SETTINGS R ALREADY NON-INTRUSIV, U SHUD CONSIDR USIN DAT OPSHUN ONLY IF U HAS ABSOLUTELY NO INTENT FRUM ASF 2 DO ANYTHIN RELATD 2 TEH INCOMIN TRADEZ AT ALL.

IZ POSIBLE 2 DISABLE DONASHUN TRADEZ FRUM OTHR BOTS, THRU `DontAcceptBotTrades` IN UR `TradingPreferences`.
