...
`byte` TYPE WIF DEFAULT VALUE OV `4`. ASF WILL ENSURE DAT THAR WILL BE AT LEAST `InventoryLimiterDelay` SECONDZ IN TWEEN OV 2 CONSECUTIV WEB INVENTORY REQUESTS 2 AVOID TRIGGERIN RATE-LIMIT - DOSE R BEAN USD 4 EXAMPLE DURIN MARKIN INVENTORY NOTIFICASHUNS AS READ, MITE ALSO BE USD BY THIRD-PARTY PLUGINS FETCHIN INVENTORY OV OTHR USERS. DIS PROPERTY IZ NOT USD 4 FETCHIN R OWN INVENTORY, AS ASF IZ USIN MUTCH MOAR EFFICIENT INTERNAL NETWORK CALL 4 DAT, SO ITLL NOT AFFECT COMMANDZ LIEK `loot` OR `transfer` IN ANY WAI. DEFAULT VALUE OV `4` WUZ SET BASD ON MARKIN INVENTORIEZ OV OVAR 100 CONSECUTIV BOT INSTANCEZ, AN SHUD SATISFY MOST (IF NOT ALL) OV TEH USERS. U CUD HOWEVR WANTS 2 DECREASE IT, OR EVEN CHANGE 2 `0` IF U HAS VRY LOW AMOUNT OV BOTS, SO ASF WILL IGNORE TEH DELAY AN MARK STEAM INVENTORIEZ MUTCH FASTR. BE WARND THOUGH, AS SETTIN IT 2 LOW **WILL** RESULT IN STEAM TEMPORARILY BANNIN UR IP, AN DAT WILL PREVENT U FRUM MAKIN ANY CALLS AT ALL. U ALSO CUD NED 2 INCREASE DIS VALUE IF URE RUNNIN LOT OV BOTS WIF LOT OV INVENTORY REQUESTS, ALTHOUGH IN DIS CASE U SHUD PROBABLY TRY 2 LIMIT NUMBR OV DOSE REQUESTS INSTEAD. UNLES U HAS **STRONG** REASON 2 EDIT DIS PROPERTY, U SHUD KEEP IT AT DEFAULT.
...
UPDATEZ OV PLUGINS R PERFORMD BY DEFAULT ALONG WIF ASF UPDATE ROUTINE - **[`UpdateChannel`](#updatechannel)** AN **[`UpdatePeriod`](#updateperiod)**. STANDARD ASF UPDATE MECHANISMS SUCH AS `update` COMMAND WILL ALSO TRIGGR OPSHUNAL PLUGINS UPDATE. IF INSTEAD UD LIEK 2 UPDATE PLUGINS MANUALLY WITHOUT UPDATIN ASF VERSHUN AT TEH SAME TIEM, `updateplugins` COMMAND OFFERS SUCH POSIBILITY.
...
BY DEFAULT ASF WILL USE ALL AVAILABLE STEAM PROTOCOLS AS MEASURE 4 FIGHTIN WIF DOWNTIMEZ AN OTHR SIMILAR STEAM ISSUEZ. TYPICALLY U WANTS 2 CHANGE DIS PROPERTY IF U WANTS 2 LIMIT ASF INTO USIN ONLY WAN OR 2 SPECIFIC PROTOCOLS. SUCH MEASUREZ CUD BE NEEDD IN VARIOUS SITUASHUNS, 4 EXAMPLE IF UVE BLOCKD UDP TRAFFIC ON UR FIREWALL AN U WANTS 2 ENSURE ONLY TCP TRAFFIC GOEZ THRU, OR IF URE USIN `WebProxy` AN WANTS 2 USE IT ALSO 4 STEAM CLIENT CONNECSHUN, AS ONLY `WebSocket` PROTOCOL SUPPORTS DAT.
...
`byte` TYPE WIF DEFAULT VALUE OV `1`. DIS PROPERTY DEFINEZ UPDATE CHANNEL WHICH IZ BEAN USD, EITHR 4 AUTO-UPDATEZ (IF `UpdatePeriod` IZ GREATR THAN `0`), OR UPDATE NOTIFICASHUNS (OTHERWIZE). CURRENTLY ASF SUPPORTS 3 UPDATE CHANNELS - `0` WHICH IZ CALLD `None`, `1`, WHICH IZ CALLD `Stable`, AN `2`, WHICH IZ CALLD `PreRelease`. `Stable` CHANNEL IZ TEH DEFAULT RELEASE CHANNEL, WHICH SHUD BE USD BY MAJORITY OV USERS. `PreRelease` CHANNEL IN ADDISHUN 2 `Stable` RELEASEZ, ALSO INCLUDEZ **PRE-RELEASEZ** DEDICATD 4 ADVANCD USERS AN OTHR DEVELOPERS IN ORDR 2 TEST NEW FEATUREZ, CONFIRM BUGFIXEZ OR GIV FEEDBACK BOUT PLANND ENHANCEMENTS. **PRERELEASE VERSHUNS OFTEN CONTAIN UNPATCHD BUGS, WERK-IN-PROGRES FEATUREZ OR REWRITTEN IMPLEMENTASHUNS**. IF U DOAN CONSIDR YOURSELF ADVANCD USR, PLZ STAY WIF DEFAULT `1` (`Stable`) UPDATE CHANNEL. `PreRelease` CHANNEL IZ DEDICATD 2 USERS HOO KNOE HOW 2 REPORT BUGS, DEAL WIF ISSUEZ AN GIV FEEDBACK - NO TECHNICAL SUPPORT WILL BE GIVEN. CHECK OUT ASF **[RELEASE CYCLE](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Release-cycle-lol-US)** IF UD LIEK 2 LERN MOAR. U CAN ALSO SET `UpdateChannel` 2 `0` (`None`), IF U WANTS 2 COMPLETELY REMOOV ALL VERSHUN CHECKZ. SETTIN `UpdateChannel` 2 `0` WILL ENTIRELY DISABLE ENTIRE FUNCSHUNALITY RELATD 2 UPDATEZ, INCLUDIN `update` COMMAND. USIN `None` CHANNEL IZ **STRONGLY DISCOURAGD** DUE 2 EXPOSIN YOURSELF 2 ALL SORT OV PROBLEMS (MENSHUND IN `UpdatePeriod` DESCRIPSHUN BELOW).
...
`string` TYPE WIF DEFAULT VALUE OV `null`. DIS PROPERTY DEFINEZ WEB PROXY ADDRES DAT WILL BE USD 4 INTERNAL HTTP-RELATD COMMUNICASHUN, ESPECIALLY 2 SERVICEZ SUCH AS `github.com`, `api.steampowered.com`, `steamcommunity.com` AN `store.steampowered.com`. IT APPLIEZ 2 GENERAL (NON-BOT SPECIFIC) COMMUNICASHUN, AS WELL AS BOT-SPECIFIC COMMUNICASHUN IF THEIR EQUIVALENT `WebProxy` CONFIGURASHUN PROPERTY IZ NOT SET. PROXYIN ASF REQUESTS CUD BE EXCEPSHUNALLY USEFUL 4 BYPASIN VARIOUS KINDZ OV FIREWALLS, ESPECIALLY TEH GREAT FIREWALL IN CHINA.
...
IF U REQUIRE PROXYIN INTERNAL STEAM NETWORK COMMUNICASHUN (CMS) AS WELL, DEN U SHUD ENSURE 2 CONFIGURE **[`SteamProtocols`](#steamprotocols)** BOTS PROPERTY 2 VALUE DAT ALLOWS ONLY WEBSOCKET TRANZPORT, I.E. VALUE OV `4`, AS ONLY WEBSOCKETS R SUPPORTD 4 PROXYIN.
...
| VALUE | NAYM                          | DESCRIPSHUN                                                                                         |
| ----- | ----------------------------- | --------------------------------------------------------------------------------------------------- |
| 0     | None                          | NO SPESHUL BOT BEHAVIOUR, SANE DEFAULT SETTINGS                                                     |
...
| 64    | DisableIncomingTradesParsing  | WILL CAUSE ASF 2 NEVR PROCES INCOMIN TRADE OFFERS                                                   |
...
IN GENERAL U WANTS 2 MODIFY DIS PROPERTY IF U EXPECT 2 CHANGE VARIOUS AUTOMASHUN SETTINGS RELATD 2 BOTS ACTIVITY. TEH DEFAULT SETTINGS INVOLVE ASF RUNNIN IN NON-INVASIV MODE, WHICH ENABLEZ ONLY BENEFISHUL AUTOMASHUN DAT DOEZ NOT GO AGAINST USERS WILL.
...
`DisableIncomingTradesParsing` WILL STOP ASF FRUM PARSIN INCOMIN TRADE OFFERS, DIS MEANZ DAT WHOLE TRADIN FUNCSHUNALITY RELATD 2 DAT WILL BE NON-OPERATIV. SINCE ASF WERKZ IN DA LEAST INVASIV MODE BY DEFAULT, ACCEPTIN ONLY TRADE OFFERS FRUM `Master` USERS AN ABOOV, NEVR TOUCHIN OTHR TRADE OFFERS - INCOMIN TRADEZ PARSIN IZ ENABLD BY DEFAULT. DIS SETTIN MAKEZ TEH MOST SENSE 4 PEEPS DAT WUD LIEK 2 ENSURE NO ADDISHUNAL REQUESTS/OVERHEAD RELATD 2 TRADEZ PARSIN, DISABLIN WHOLE LOGIC IN PROCES, 4 EXAMPLE CUZ THEY KNOE DAT THEIR BOTS WILL NEVR RECEIV MASTAH TRADE REQUESTS, AN THEREFORE DO NOT REQUIRE ASF PARTICIPATIN IN THEIR TRADIN ACTIVITY AT ALL. KEEP IN MIND DAT HAVIN DIS OPSHUN SPECIFID WILL DISABLE ALL OTHR OPSHUNS DAT DEPEND ON INCOMIN TRADEZ AS WELL, SUCH AS `AcceptDonations` OR `SteamTradeMatcher` - CUSTOM PLUGINS WILL ALSO BE UNABLE 2 PROCES INCOMIN TRADE OFFERS IN USUAL WAI.
...
`AutoUnpackBoosterPacks` WILL AUTOMATICALLY UNPACK ALL BOOSTR PACKZ UPON RECEIVIN NEW ITEMS NOTIFICASHUN. DIS WILL ALLOW U 2 IMMEDIATELY ACQUIRE ADDISHUNAL CARD DROPS, WHICH MITE BE WANTD SCENARIO ESPECIALLY IN COMBINASHUN WIF OTHR OPSHUNS (E.G. `SteamTradeMatcher` OR `CompleteTypesToSend`).
...
`byte flags` TYPE WIF DEFAULT VALUE OV `0`. DIS PROPERTY CAN ENABLE SUM ADDISHUNAL ONLINE FEATUREZ ON TEH STEAM PLATFORM, AN IZ DEFIND AS BELOW:

| VALUE | NAYM        | DESCRIPSHUN                            |
| ----- | ----------- | -------------------------------------- |
| 0     | None        | NO SPESHUL ONLINE PREFERENCEZ, DEFAULT |
...
`byte` TYPE WIF DEFAULT VALUE OV `0`. DIS PROPERTY SPECIFIEZ USR INTERFACE MODE DAT TEH BOT WILL BE ANNOUNCD WIF AFTR LOGGIN IN 2 TEH STEAM NETWORK. IT MITE INFLUENCE HOW TEH AKOWNT IZ VISIBLE E.G. ON TEH STEAM CHAT, IF UR PRESENCE ALLOWS DAT THRU `OnlineStatus`. CURRENTLY U CAN CHOOSE WAN OV BELOW MODEZ:

| VALUE | NAYM           | DESCRIPSHUN               |
| ----- | -------------- | ------------------------- |
| `0`   | VGUI           | DEFAULT STEAM CLIENT MODE |
| `1`   | Tenfoot        | HOOJ PICCHUR MODE         |
| `2`   | Mobile         | STEAM MOBILE APP          |
| `3`   | Web            | WEB BROWSR SESHUN         |
| `4`   | ClientUI       |                           |
| `5`   | MobileChat     | STEAM MOBILE CHAT APP     |
| `6`   | EmbeddedClient |                           |

NOT ALL MODEZ MITE HAS AN EFFECT. ALSO, U MITE BE INTERESTD IN CHECKIN `OnlinePreferences`, SINCE SUM ADDISHUNAL FEATUREZ R ENABLD THAR. IF URE NOT SURE HOW 2 SET DIS PROPERTY, LEEF IT WIF DEFAULT VALUE OV `0`.

---

### `WebProxy`

`string` TYPE WIF DEFAULT VALUE OV `null`. DIS PROPERTY DEFINEZ WEB PROXY ADDRES DAT WILL BE USD 4 BOT-SPECIFIC INTERNAL HTTP-RELATD COMMUNICASHUN, ESPECIALLY 2 SERVICEZ SUCH AS `api.steampowered.com`, `steamcommunity.com` AN `store.steampowered.com`. IF NOT SET, ASF WILL USE GLOBAL `WebProxy` SETTIN SPECIFID ABOOV INSTEAD. PROXYIN ASF REQUESTS CUD BE EXCEPSHUNALLY USEFUL 4 BYPASIN VARIOUS KINDZ OV FIREWALLS, ESPECIALLY TEH GREAT FIREWALL IN CHINA.
