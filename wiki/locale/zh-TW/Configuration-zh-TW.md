...
啟用這個模式將允許您透過其他方式提供所需的控制台輸入，也就是 ASF-ui（ASF API），或透過&#8203;**[`input`](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Commands-zh-TW#input-指令)**&#8203;指令。 若您不確定如何設定本屬性，請保留預設值&#8203;`false`&#8203;。
...
`byte`&#8203;型別，預設值為&#8203;`4`&#8203;。 ASF會確保在兩次連續的物品庫請求間至少間隔&#8203;`InventoryLimiterDelay`&#8203;秒，以避免觸發速率限制⸺請求的使用，有可能是將物品庫通知設為已讀，也可能是第三方外掛程式用於提取其他使用者的物品庫。 本屬性並非用於提取自己的物品庫，因為ASF有更高效的內部網路呼叫來達成，因此這並不會影響如&#8203;`loot`&#8203;或&#8203;`transfer`&#8203;的功能。 預設值&#8203;`4`&#8203;是依據連續標示超過100個Bot實例的物品庫所設定的，應該能滿足大部分（可能不是全部）的使用者需求。 但是，如果您的Bot數量很少，您可能會想要減少它，或甚至把它更改成&#8203;`0`&#8203;，使ASF忽略延時並盡速標示Steam物品庫。 但請注意，設定過低的值&#8203;**將會**&#8203;導致Steam暫時封鎖您的IP，並完全阻止您標示您的物品庫。 若您執行了很多需要大量物品庫請求的Bot，您可能還會需要增加此值，但在這種情形下，您可能更應該去限制這些請求的數量。 除非您有&#8203;**充分的**&#8203;理由編輯此屬性，否則您應維持它為預設值。
...
在預設情形下，外掛程式的更新會與ASF的&#8203;**[`UpdateChannel`](#updatechannel)**&#8203;及&#8203;**[`UpdatePeriod`](#updateperiod)**&#8203;設定一同更新。 標準ASF更新行為如&#8203;`update`&#8203;指令也可以觸發外掛程式的更新。 若您只想手動更新外掛程式，而不同時更新ASF的版本，可以使用&#8203;`updateplugins`&#8203;指令。

除非您有理由編輯此屬性，否則您應維持它為預設值。

---

### `ShutdownIfPossible（在可能時關閉）`

`bool`&#8203;型別，預設值為&#8203;`false`&#8203;。 啟用時，ASF將會在能夠的時候，也就是所有登錄的Bot都停止時，嘗試關閉程序。 這在與您所有Bot實例的&#8203;`ShutdownOnFarmingFinished`&#8203;相結合使用時會特別有用，這樣在您的最後一個Bot完成掛卡時，ASF將被允許自動關閉。

因為大多數使用者都會期望程序始終處於執行狀態，例如在使用&#8203;`IPC`&#8203;時，所以本選項預設為停用。
...
預設情形下，ASF會使用所有可以使用的Steam協定，作為應對當機期間及其他相似的Steam問題的措施。 若您想要限制ASF只使用一兩種指定的協定，您需要修改本屬性。 這些設定會在不同的情境中被使用，例如您在防火牆上封鎖了UDP流量，並確保只有TCP流量可以通過；或您使用了&#8203;`WebProxy`&#8203;並希望也用在Steam用戶端連線上，則要使用&#8203;`WebSocket`&#8203;，因為只有該協定支援代理。
...
`byte`&#8203;型別，預設值為&#8203;`1`&#8203;。 本屬性定義了ASF使用的更新通道，用於自動更新（如果&#8203;`UpdatePeriod`&#8203;大於&#8203;`0`&#8203;時），或更新通知（&#8203;`UpdatePeriod`&#8203;為&#8203;`0`&#8203;時）。 目前ASF支援三個更新通道：&#8203;`0`&#8203;為&#8203;`None`&#8203;；&#8203;`1`&#8203;為&#8203;`Stable`&#8203;；而&#8203;`2`&#8203;為&#8203;`PreRelease`&#8203;。 `Stable`&#8203;通道是預設的發布通道，這適合大多數的使用者。 `PreRelease`&#8203;除了&#8203;`Stable`&#8203;版本以外，也包含了專門提供進階使用者及其他開發人員測試新功能、確認錯誤修復或提供相關增強計畫回饋的&#8203;**預覽版本**&#8203;。 **預覽版本通常包含了尚未修補的錯誤、未完成或重寫的功能**&#8203;。 若您並非進階使用者，請保留預設值&#8203;`1`&#8203;（&#8203;`Stable`&#8203;）更新通道。 `PreRelease`&#8203;通道是專門提供給知道如何回報錯誤、處理問題並給予回饋的使用者所使用⸺我們不會提供任何技術支援。 若您想深入了解更多資訊，請參閱&#8203;**[發布週期](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Release-cycle-zh-TW)**&#8203;。 若您想完全停用所有版本檢查，您也可以將&#8203;`UpdateChannel`&#8203;設定成&#8203;`0`&#8203;（&#8203;`None`&#8203;）。 將&#8203;`UpdateChannel`&#8203;設定成&#8203;`0`&#8203;會完全停用與更新相關的功能，包含&#8203;`update`&#8203;指令。 **強烈反對**&#8203;您使用&#8203;`None`&#8203;通道，因為這會使您自己面臨各種問題（在下文&#8203;`UpdatePeriod`&#8203;的說明中有提到）。
...
`string`&#8203;型別，預設值為&#8203;`null`&#8203;。 本屬性定義了網頁代理位址，內部HTTP相關通訊會透過這個代理，特別是例如&#8203;`github.com`&#8203;、&#8203;`steamcommunity.com`&#8203;、&#8203;`store.steampowered.com`&#8203;及&#8203;`steamcommunity.com`&#8203;服務。 它用於一般（非Bot特定的）通訊，以及如果Bot未使用&#8203;`WebProxy`&#8203;等效設定屬性時的特定通訊。 對ASF的請求使用代理對於繞過各式各樣的防火牆來說特別有用，特別是中國的長城防火牆。
...
若您也需要代理內部的Steam網路通訊（CM），那麼您應確保將&#8203;**[`SteamProtocols`](#steamprotocols)**&#8203; Bot屬性設定成只允許Websocket傳輸的值，也就是&#8203;`4`&#8203;，因為只有Websocket支援代理。
...
| 64 | DisableIncomingTradesParsing（停用收到的交易剖析） | 使ASF將永不處理收到的交易提案                       |

請注意，本屬性為&#8203;`flags`&#8203;欄位，因此可以使用所有可用值任意組合。 若您想深入了解更多資訊，請參閱&#8203;**[JSON映射](#json-映射)**&#8203;。 不啟用任何旗標即為&#8203;`None`&#8203;選項。

在一般情形下，如果您想要調整與Bot行為相關的自動化設定，就需要修改這個屬性。 預設設定僅啟用不違背使用者意願，且有益的自動化行為，包含使用非侵入模式執行ASF。
...
`DisableIncomingTradesParsing`&#8203;將會阻止ASF剖析收到的交易提案，這代表與其相關的交易功能將無法使用。 因ASF在預設情形下以最低侵入性模式運作，所以僅接受來自&#8203;`Master`&#8203;權限或以上使用者的交易提案，忽略其他所有交易提案⸺所以交易剖析預設是啟用的。 這項設定對那些不產生額外交易提案請求／負擔的使用者來說最實用，例如他們知道他們的Bot永遠不會收到來自Master的交易請求，因此也就不需要ASF參與至那些交易行為中。 請注意，使用本選項將停用所有依靠接收交易提案的功能，例如&#8203;`AcceptDonations`&#8203;或&#8203;`SteamTradeMatcher`&#8203;⸺自訂外掛程式也將無法以一般的方式處理收到的交易提案。
...
| 256 | AutoUnpackBoosterPacks（自動打開擴充包）         |

請注意，本屬性為&#8203;`flags`&#8203;欄位，因此可以使用所有可用值任意組合。 若您想深入了解更多資訊，請參閱&#8203;**[JSON映射](#json-映射)**&#8203;。 不啟用任何旗標即為&#8203;`None`&#8203;選項。

所有選項的解說如下：

`FarmingPausedByDefault`&#8203;定義了&#8203;`CardsFarmer`&#8203;模組的初始狀態。 通常Bot會在經由&#8203;`Enabled`&#8203;或使用&#8203;`start`&#8203;指令啟動時，自動開始掛卡。 若您希望手動&#8203;`resume`&#8203;自動掛卡程序，可以使用&#8203;`FarmingPausedByDefault`&#8203;。例如您只想使用&#8203;`play`&#8203;，且永遠不會使用自動&#8203;`CardsFarmer`&#8203;模組⸺這將與&#8203;`pause`**[指令](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Commands-zh-TW)**&#8203;的作用完全相同。

`ShutdownOnFarmingFinished`&#8203;使您能夠在掛卡完成時關閉Bot。 通常ASF會在程序執行過程中持續「占用」帳號。 在指定帳號完成掛卡後，ASF會（每隔&#8203;`IdleFarmingPeriod`&#8203;小時）定期檢查，若在該段時間帳號新增了一些帶有Steam交換卡片的遊戲，那麼ASF能夠繼續掛該帳號的卡，而不用重啟整個程序。 這對於大多數人來說應該很有幫助，因為ASF能夠在需要時自動恢復掛卡。 但是，您也可能會希望在指定帳號掛卡完成後停止程序，這就需要設定本屬性。 啟用後，ASF將會在掛卡全數完成後登出帳號，這代表該帳號將不再被定期檢查或佔用。 您應該自行決定，會更希望ASF始終對Bot實例運作，或希望ASF在掛卡程序完成後停止它。

本選項與&#8203;`ShutdownIfPossible`&#8203;一起使用時才會最有用，在所有帳號都終止運作時，ASF也會自行關閉，讓您的設備休息，使您能夠安排其他操作，例如在最後一張交換卡片掉落後睡眠或關機。

`SendOnFarmingFinished`&#8203;使您能夠自動將到目前為止所掛出的卡，使用交易提案發送給具有&#8203;`Master`&#8203;權限的使用者，若您不想自行發送交易提案，這個功能會對您有所幫助。 本選項與&#8203;`loot`&#8203;指令作用相同，因此請注意，首先它需要您有帳號的交易合法權限，且使用者具有&#8203;`Master`&#8203;權限，而您可能也要有有效的&#8203;`SteamTradeToken`&#8203;。 在本選項啟用時，ASF除了會在掛卡完成後執行&#8203;`loot`&#8203;外，也會在（未掛卡時）每次收到新物品通知後，及在完成產生新物品的交易後執行&#8203;`loot`&#8203;。 這在您把來自其他人的物品「轉發」到自己的帳號中時特別有用。 通常您會需要同時使用&#8203;**[ASF雙重驗證](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Two-factor-authentication-zh-TW)**&#8203;，但若您傾向於手動確認雙重驗證，則這並不是必需的。

`FarmPriorityQueueOnly`&#8203;定義了ASF是否只自動掛卡您使用&#8203;`fq`&#8203;**[指令](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Commands-zh-TW)**&#8203;加入至優先掛卡佇列中的應用程式。 在啟用本選項時，ASF將會跳過所有不在清單中的&#8203;`appIDs`&#8203;，使您能夠挑選您想要ASF自動掛卡的遊戲。 請注意，若您沒有在佇列中加入任何遊戲，ASF會表現得像您帳號中沒有任何遊戲可以掛卡一樣。

`SkipRefundableGames`&#8203;定義了ASF在自動掛卡時，是否應跳過那些仍可退款的遊戲。 所謂可退款的遊戲，是您在過去2週內透過Steam商店購買、且遊玩時數不超過2小時的遊戲，如&#8203;**[Steam退款](https://store.steampowered.com/steam_refunds)**&#8203;頁面所述。 預設情形下，ASF會完全忽略Steam退款政策，掛所有能掛的遊戲，而這也是大多數人所想要的。 但是，如果您想確保ASF不會過快掛您的可退款遊戲，可以啟用這項旗標，使您能夠自行體驗遊戲，並在需要時退款，避免ASF影響您的遊玩時數。 請注意，如果您啟用本選項，會從您在Steam商店購買遊戲的14日內，ASF將不會掛該遊戲，若在此期間您的帳號沒有其他可掛的遊戲，ASF將會顯示為閒置。
...
`AutoUnpackBoosterPacks`&#8203;會在收到新物品通知時，自動拆開所有擴充包。 這使您能夠立即獲得額外的卡片掉落，而這可能正是您所需的，特別是在與其他選項（例如&#8203;`SteamTradeMatcher`&#8203;或&#8203;`CompleteTypesToSend`&#8203;）結合使用的情形下。
...
### `OnlinePreferences（在線偏好）`

`byte flags`&#8203;型別，預設值為&#8203;`0`&#8203;。 本屬性可以在Steam平台啟用一些額外的在線功能，定義如下：

| 值 | 名稱                       | 描述               |
| - | ------------------------ | ---------------- |
| 0 | 無                        | 預設值，無特殊在線偏好      |
| 1 | IsSteamDeck（為 SteamDeck） | 將自身當作 Steam Deck |
...
`byte`&#8203;型別，預設值為&#8203;`0`&#8203;。 本屬性指定了Bot在登入Steam網路後使用的使用者介面模式。 它可能會影響帳號的可見度，例如在Steam聊天中（若您的偏好設定透過了&#8203;`OnlineStatus`&#8203;允許）。 目前您可以選擇下列模式之一：

| 值   | 名稱                     | 描述               |
| --- | ---------------------- | ---------------- |
| `0` | VGUI                   | 預設的Steam用戶端模式    |
| `1` | Tenfoot                | Big Picture 模式   |
| `2` | Mobile（行動裝置）           | Steam 行動裝置應用程式   |
| `3` | Web（網頁）                | 網頁瀏覽器連線階段        |
| `4` | ClientUI（用戶端介面）        |                  |
| `5` | MobileChat（行動裝置聊天）     | Steam 行動裝置聊天應用程式 |
| `6` | EmbeddedClient（嵌入式用戶端） |                  |

並非所有模式都有效果。 此外，您可能有興趣檢查&#8203;`OnlinePreferences`&#8203;，因為那裡也啟用了一些額外功能。 若您不確定如何設定本屬性，請保留預設值&#8203;`0`&#8203;。

---

### `WebProxy（網頁代理）`

`string`&#8203;型別，預設值為&#8203;`null`&#8203;。 本屬性定義了網頁代理位址，Bot特定的內部HTTP相關通訊會透過這個代理，特別是例如&#8203;`steamcommunity.com`&#8203;、&#8203;`store.steampowered.com`&#8203;及&#8203;`steamcommunity.com`&#8203;服務。 若未指定，則ASF會使用上述指定的全域&#8203;`WebProxy`&#8203;設定。 對ASF的請求使用代理對於繞過各式各樣的防火牆來說特別有用，特別是中國的長城防火牆。
...
`log.txt`&#8203;檔案儲存著您上一次執行ASF時所產生的紀錄日誌。 本檔案並未包含任何敏感性資訊，在與問題、崩潰相關，或提供上次執行ASF時的資訊時非常有用。 若您遇到了問題或錯誤，我們經常會需要這份檔案。 ASF會自動為您管理這份檔案，但如果您是進階使用者，您也可以進一步調整ASF的&#8203;**[紀錄日誌](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Logging-zh-TW)**&#8203;模組。
