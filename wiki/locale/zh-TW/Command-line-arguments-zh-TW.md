...
`--cryptkey-file<path>`&#8203;或&#8203;`--cryptkey-file=<path>`&#8203;⸺將使用從&#8203;`<path>`&#8203;檔案中讀取的自訂加密金鑰&#8203;`<path>`&#8203;啟動ASF。 這與上述說明的&#8203;`--cryptkey<key>`&#8203;的目的相同。但機制不同，因為此屬性是從&#8203;`<path>`&#8203;讀取&#8203;`<key>`&#8203;。 若您將本選項與&#8203;`--path`&#8203;一起使用，需注意相對路徑會依引數的順序而有所不同，例如，將&#8203;`--path`&#8203;放置在&#8203;`--cryptkey-file`&#8203;的前面或後面行為將會不同。
...
`--minimized`&#8203;⸺使ASF在啟動後立刻最小化控制台視窗。 主要是在自動啟動的情境中非常好用，但仍可用於其他地方。 此選項需要正確的環境支援⸺可能無法於所有情境中正常運作。
...
`--system-required`&#8203;⸺使用這個開關，將使ASF嘗試通知作業系統，表明本程序需要在系統執行期間，保持啟動與正常執行狀態。 目前這個開關只對Windows設備有效，只要程序正在執行，它就會阻止您的系統進入睡眠模式。 當您在晚上使用PC或筆電掛卡時，這將會特別有用，因為ASF會在執行時維持系統在甦醒狀態。