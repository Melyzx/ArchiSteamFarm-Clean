...
- `linux-arm64` é compatível com sistemas operacionais GNU/Linux 32-bit baseados em ARM (ARMv7 +) com glibc 2.35/musl 1.2.2 e posteriores. Esta variante cobre plataformas como o Raspberry Pi 2 (e mais novos), ela **não** funcionará com arquiteturas ARM antigas, como ARMv6 do Raspberry Pi 0 & 1, e também é incompatível com sistemas que não implementem o ambiente GNU/Linux exigido (como o sistema Android).
...
- `osx-arm64` compatível com o OS X de 64 bits baseados em ARM (Apple silicon) na versão 13 e posteriores.
- `osx-x64` compatível com o OS X 64-bit na versão 13 e posteriores.
...
Para obter uma lista completa de todas as plataformas e Sistemas Operacionais compatíveis com o .NET 9.0, visite as **[notas de lançamento](https://github.com/dotnet/core/blob/main/release-notes/9.0/supported-os.md)**.
...
O ASF, como programa, está atualmente direcionado ao **.NET 9.0** (`net9.0`), mas pode adotar uma plataforma mais recente no futuro. `net9.0` é compatível desde o SDK 9.0.100 (runtime 9.0.0), embora o ASF possa preferir **o runtime mais recente no momento da compilação**. Por isso, você deve garantir que possui o **[SDK mais recente](https://dotnet.microsoft.com/download)** (ou pelo menos o runtime) disponível para sua máquina. A variante genéria do ASF pode se recusar a iniciar se o seu tempo de execução for mais antigo que o mínimo especificado durante a compilação.
