...
* **[Configuração do ASF-ui](#configuração-da-interface-do-asf)**
...
* **[Compatibilidade de configurações](#configuração-de-compatibilidade)**
* **[Reinício automático](#recarregamento-automático)**

---

## Introdução

A configuração do ASF é dividida em duas partes principais: configuração global (do processo) e configuração individual dos bots. Cada bot tem seu próprio arquivo de configuração chamado `NomeBot.json` (onde `NomeBot` é o nome dado ao bot), enquanto as configurações globais (do processo) do ASF estão em um único arquivo chamado `ASF.json`.

Chamamos de bot qualquer conta Steam usada no processo do ASF. Para funcionar corretamente, o ASF precisa de pelo menos **uma** instância de bot definida. Não há limite de instâncias de bot imposto pelo processo, então você pode usar quantos bots (contas Steam) desejar.

O ASF está utilizando o formato **[JSON](https://pt.wikipedia.org/wiki/JSON)** para armazenar seus arquivos de configuração. É um formato amigável para humanos, legível e muito universal, no qual você pode configurar o programa. Porém não se preocupe, você não precisa saber JSON para configurar o ASF. Apenas mencionei isso caso você já queira criar em massa as configurações do ASF com algum tipo de script bash.

A configuração pode ser feita de várias formas. Você pode usar nosso **[Gerador de configuração web](https://justarchinet.github.io/ASF-WebConfigGenerator)**, que é um aplicativo local independente do ASF. Você pode usar nossa interface IPC **[ASF-ui](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/IPC-pt-BR#asf-ui)** para configurar diretamente no ASF. Por último, você sempre pode gerar arquivos de configuração manualmente, pois eles seguem uma estrutura JSON fixa especificada abaixo. Explicaremos brevemente as opções disponíveis.
...
O uso é muito simples: selecione se você deseja gerar uma configuração `ASF` ou `Bot` alternando para a guia adequada, certifique-se de que a versão do arquivo de configuração corresponde a sua versão do ASF, então insira todos os dados e clique no botão "download". Mova este arquivo para a pasta `config` do ASF, substituindo os arquivos existentes, caso necessário. Repita para todas as modificações eventuais futuras e consulte o restante desta seção para explicação de todas as opções disponíveis para configurar.

---

## Configuração da Interface do ASF

Nossa interface IPC **[ASF-ui](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/IPC-pt-BR#asf-ui)** também permite que você configure o ASF, e é uma solução superior para reconfigurar o ASF após gerar as configurações iniciais devido ao fato de que ele pode editar as configurações durante a execução, ao contrário do Gerador de Configuração web que as gera estaticamente.

Para usar o ASF-ui você deve habilitar nossa interface **[IPC](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/IPC-pt-BR)**. O `IPC` está ativado por padrão, portanto você pode acessá-lo imediatamente, desde que não o tenha desativado por conta própria.
...
Tipo `byte` com o valor padrão `4`. O ASF garantirá que haja pelo menos `InventoryLimiterDelay` segundos entre duas solicitações consecutivas do inventário na web para evitar o acionamento do limitador de tráfego — essas são usadas, por exemplo, durante a marcação de notificações de inventário como lidas, podendo também ser usadas por plugins de terceiros que buscam o inventário de outros usuários. Essa propriedade não é usada para obter o nosso próprio inventário, já que o ASF utiliza uma chamada de rede interna muito mais eficiente para isso, então não afetará comandos como `loot` ou `transfer` de forma alguma. O valor padrão de `4` foi definido com base na marcação de inventários de mais de 100 instâncias consecutivas de bots, e deve satisfazer a maioria (se não todos) dos usuários. No entanto, você pode querer diminuir esse valor, ou até mesmo alterá-lo para `0` se tiver uma quantidade muito pequena de bots, para que o ASF ignore o atraso e marque os inventários do Steam muito mais rápido. No entanto, seja avisado de que, caso definir um valor muito baixo, **o** Steam bloqueará temporariamente o seu IP, o que impedirá que você faça qualquer chamada. Você talvez precise aumentar esse valor se estiver executando muitos bots com muitos pedidos de acesso ao inventário, embora neste caso você deva provavelmente tentar limitar o número dessas solicitações em vez disso. A menos que você tenha uma razão muito **forte** para editar essa propriedade, você deve mantê-la padrão.
...
Tipo `byte` com o valor padrão `0`. Esta propriedade define o modo de atualização de plugins que dá propósito à `PluginsUpdateList` definida acima. Ao especificar esta propriedade, você pode facilmente habilitar/desabilitar atualizações automáticas para todos os plugins, exceto aqueles declarados.

- O valor de `0`, chamado de `Whitelist`, **desabilita** a atualização automática de todos os plugins, exceto aqueles definidos em `PluginsUpdateList`.
- O valor de `1`, chamado de `Blacklist`, **habilita** a atualização automática de todos os plugins, exceto aqueles definidos em `PluginsUpdateList`.

**A equipe do ASF gostaria de lembrá-lo que, para sua própria segurança, você deve habilitar atualizações automáticas apenas de desenvolvedores confiáveis**. Tenha em mente que plugins maliciosos podem decidir se atualizar ou executar comandos remotos **independentemente** dessa configuração, por isso, essa configuração se aplica exclusivamente à funcionalidade de atualização de plugins fornecida pelo ASF, e você ainda deve garantir que tenha verificado adequadamente cada plugin que decidiu usar.

As atualizações dos plugins são realizadas por padrão juntamente com a rotina de atualização do ASF - **[`UpdateChannel`](#updatechannel)** e **[`UpdatePeriod`](#updateperiod)**. Os mecanismos de atualização padrão do ASF, como o comando `update`, também acionarão a atualização opcional dos plugins. Caso, em vez disso, você desejar atualizar os plugins manualmente sem atualizar a versão do ASF ao mesmo tempo, o comando `updateplugins` oferece essa possibilidade.
...
Tipo `bool` com o valor padrão `false`. Quando habilitado, o ASF tentará desligar o processo se possível, isto é, quando todos os seus bots registrados forem interrompidos. Isso pode ser especialmente útil quando combinado com `ShutdownOnFarmingFinished` em todos os seus bots, já que dessa forma o ASF será autorizado a desligar automaticamente quando o último dos seus bots terminar a coleta.

Uma vez que a expectativa da maioria dos usuários é de que o processo funcione em todos os momentos, para usar o `IPC` por exemplo, esta opção vem desativada por padrão.
...
Tipo `byte` com o valor padrão `1`. Essa propriedade define o canal de atualização que está sendo usado, tanto para atualizações automáticas (se `UpdatePeriod` for maior que `0`), ou (caso contrário) para atualizar notificações. Atualmente, o ASF suporta três canais de atualização: `0`, chamado de `None`, `1`, chamado de `Stable`, e `2`, chamado de `PreRelease`. O canal `Estável` é o canal de lançamento padrão, que deve ser usado pela maioria dos usuários. O canal `PreRelease`, além das versões `Stable`, também inclui **pré-lançamentos** dedicados a usuários avançados e outros desenvolvedores para testar novas funcionalidades, confirmar correções de bugs ou dar feedback sobre melhorias planejadas. **As versões PreRelease frequentemente contêm bugs não corrigidos, funcionalidades em desenvolvimento ou implementações reescritas**. Se você não se considera um usuário avançado, por favor, mantenha o canal de atualização padrão `1` (`Estável`). O canal `PreRelease` é dedicado a usuários que sabem como relatar bugs, lidar com problemas e fornecer feedback – nenhum suporte técnico será oferecido. Confira o **[ciclo de lançamento](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Release-cycle-pt-BR)** do ASF se você quiser aprender mais. Você também pode definir `UpdateChannel` como `0` (`Nenhum`), se você quiser remover completamente todas as verificações de versão. Definir `UpdateChannel` como `0` desativará inteiramente toda funcionalidade relacionada às atualizações, incluindo o comando `update`. Usando o canal `Nenhum` é **fortemente desencorajado** devido ao fato de te expor a todo tipo de problemas (mencionados na descrição de `UpdatePeriod` abaixo).
...
| Valor | Nome                          | Descrição                                                                                                                     |
| ----- | ----------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| 0     | None                          | No special bot behaviour, sane default settings                                                                               |
...
`AutoUnpackBoosterPacks` irá desempacotar automaticamente todos os pacotes bônus ao receber a notificação de novos itens. Isso permitirá que você adquira imediatamente cartas adicionais, o que pode ser um cenário desejado, especialmente em combinação com outras opções (por exemplo, `SteamTradeMatcher` ou `CompleteTypesToSend`).
...
| 3     | FoilTradingCard       | Versão brilhante de `TradingCard`                                                       |
...
| 6     | SteamGems             | Gemas do Steam usadas para criar boosters (pacotes de cartas), incluindo as empacotadas |
| 7     | SaleItem              | Itens especiais ganhos durante as promoções Steam                                       |
| 8     | Consumable            | Itens consumíveis especiais que desaparecem após serem usados                           |
| 9     | ProfileModifier       | Itens especiais que podem modificar a aparência do Perfil Steam                         |
| 10    | Sticker               | Itens especiais que podem ser usados na Conversa Steam                                  |
...
Tipo `byte flags` com o valor padrão `0`. Esta propriedade pode habilitar alguns recursos online adicionais na plataforma Steam e é definida como abaixo:

| Valor | Nome        | Descrição                                 |
| ----- | ----------- | ----------------------------------------- |
| 0     | None        | Sem preferências especiais online, padrão |
| 1     | IsSteamDeck | Apresente-se como Steam Deck              |
...
Normalmente, você vai querer usar o **[ASF 2FA](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Two-factor-authentication-pt-BR)** junto com essa funcionalidade, embora não seja um requisito se você pretende lidar manualmente com as confirmações de 2FA de maneira oportuna. Se você não tem certeza de como definir essa propriedade, deixe-a com o valor padrão `0`.
...
Recomendamos que defina exatamente um usuário como `Master` e quantos você quiser como `Operators` e abaixo. Embora seja tecnicamente possível definir vários `Masters` e o ASF funcionará corretamente com eles, por exemplo, aceitando todas as trocas enviadas para o bot por eles, o ASF usará apenas um deles (com o menor ID do Steam) para cada ação que requer um único alvo, como uma solicitação de `loot`, portanto, também propriedades como a preferência `SendOnFarmingFinished` em `FarmingPreferences` ou `SendTradePeriod`. Se você entende perfeitamente essas limitações, especialmente o fato de que a solicitação de `loot` sempre enviará os itens para o `Master` com ID Steam mais baixo, independente do `Master` que tenha executado o comando, então você pode definir vários usuários com a permissão `Master` aqui, mas ainda assim recomendamos um único master.
...
| 16    | MatchActively       | Participa ativamente de trocas do tipo **[STM](https://www.steamtradematcher.com)**. Visite a seção **[ItemsMatcherPlugin](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/ItemsMatcherPlugin-pt-BR#matchactively)** para obter mais informações |

Por favor note que esta propriedade é um campo do tipo `flags`, portanto é possível escolher qualquer combinação de valores disponíveis. Confira o **[mapeamento json](#mapeamento-json)** se você quiser aprender mais. Não habilitar nenhum flag resultará na opção `None`.
...
Tipo `byte` com o valor padrão `0`. Esta propriedade especifica o modo de interface do usuário com o qual o bot será anunciado após fazer login na rede Steam. Isso pode influenciar como a conta é visível, por exemplo, no chat do Steam, se a sua presença permitir isso através de `OnlineStatus`. Atualmente você pode escolher um dos modos abaixo:

| Valor | Nome           | Descrição                      |
| ----- | -------------- | ------------------------------ |
| `0`   | VGUI           | Modo cliente Steam padrão      |
| `1`   | Tenfoot        | Modo Big Picture               |
| `2`   | Mobile         | Aplicativo móvel Steam         |
| `3`   | Web            | Sessão do navegador da Web     |
| `4`   | ClientUI       |                                |
| `5`   | MobileChat     | Aplicativo de chat móvel Steam |
| `6`   | EmbeddedClient |                                |

Nem todos os modos podem ter algum efeito. Além disso, você pode se interessar em verificar `OnlinePreferences`, já que alguns recursos adicionais são habilitados lá. Caso você não tem certeza de como definir essa propriedade, deixe-a com o valor padrão `0`.
...
O ASF está ciente de configurações sendo modificadas "em tempo real" - graças a isso, o ASF automaticamente:
- Criar (e iniciar, caso necessário) um novo bot, quando você criar a configuração do mesmo
- Parar (caso necessário) e remover o bot antigo, quando você excluir a sua configuração
- Parar (e iniciar, caso necessário) qualquer bot, quando você editar a configuração do mesmo
- Reiniciar (caso necessário) o bot com novo nome, quando você renomear sua configuração
