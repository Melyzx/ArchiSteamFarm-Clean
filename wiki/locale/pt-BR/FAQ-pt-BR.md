...
- **Seu jogo não pode estart marcado como [privado](https://help.steampowered.com/faqs/view/1150-C06F-4D62-4966), o ASF automaticamente ignorará esses jogos durante a coleta de cartas.**
...
A melhor maneira para isso é configurar **[`GamesPlayedWhileIdle`](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Configuration-pt-BR#gamesplayedwhileidle)**, que vai rodar seus appIDs escolhidos quando o ASF não tiver cartas para coletar. Caso você queira jogar seus jogos o tempo todo, mesmo que tenha drops de cartas de outros jogos, você pode combiná-los com **[`FarmPriorityQueueOnly`](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Configuration-pt-BR#farmingpreferences)**, para que o ASF farme apenas os jogos que você definiu explicitamente para drops de cartas ou, alternativamente, com **[`FarmingPausedByDefault`](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Configuration-pt-BR#farmingpreferences)**, o que fará com que o módulo de coleta de cartas fique pausado até que você o reative manualmente.

Você também pode fazer uso do comando **[`play`](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Commands-pt-BR#comandos-1)**, que fará com que o ASF jogue seus jogos selecionados. No entanto, tenha em mente que `play` deve ser usado apenas para jogos que você deseja rodar temporariamente, pois como ele não é um estado persistente o ASF reverterá para o estado padrão após desconexão da Rede Steam, por exemplo. Portanto, recomendamos que você use o `GamesPlayedWhileIdle`, a menos que você realmente deseje iniciar seus jogos selecionados apenas por um curto período de tempo e depois reverter para o fluxo padrão.
...
Não, isso não é possível porque o ASF (diferente do Idle Master ou SAM) não interfere com o cliente Steam nem com os seus processos. É fisicamente impossível tomar um banimento VAC por usar o ASF, mesmo jogando em servidores seguros enquanto o ASF estiver rodando - isso porque **o ASF nem mesmo precisa que o cliente Steam esteja instalado** para funcionar corretamente. O ASF é um dos únicos programas de coleta que pode garantir ser livre de banimento VAC.
...
Portanto, assim como todos os softwares da Valve, o ASF não é autorizado por ela e não posso garantir que você não vai ser suspenso se a Valve decidir de repente que vão banir contas que usem o ASF. Isso é muito improvável, considerando o fato de que o ASF está sendo usado em mais de um milhão de contas Steam, mas ainda é uma possibilidade, independentemente da probabilidade real.
...
Você está usando este software por sua conta e risco. É extremamente improvável que você possa ser banido por isso, mas caso aconteça, você pode culpar apenas a si mesmo por isso.
...
Todos os incidentes acima têm uma coisa em comum — o ASF é apenas uma ferramenta, e a decisão de como usá-la é **sua**. Você não é banido por usar o ASF diretamente, mas por **como** você o utiliza. Ele pode ser uma ferramenta auxiliar para farmar apenas uma única conta ou uma rede massiva de farming composta por milhares de bots. Em qualquer caso, não estou oferecendo aconselhamento legal, e você deve decidir por si mesmo sobre o uso do ASF em primeiro lugar. Não estou escondendo nenhuma informação que possa ajudá-lo, como o fato de que o ASF já levou algumas pessoas a serem banidas (e em qual contexto), pois não tenho motivo para isso — a escolha do que fazer com essa informação é sua.
...
### Eu não quero que o ASF colete cartas de nenhum jogo por padrão, mas ainda quero usar os recursos extras dele. É possível?

Sim, caso você queira iniciar o ASF com o módulo de coleta de cartas pausado, você pode definir `FarmingPausedByDefault` na propriedade de configuração do bot `FarmingPreferences` para conseguir isso. Isso permitirá que você `retome` a coleta durante o tempo de execução.
...
Simplesmente feche o processo ASF, por exemplo, clicando em [X] no Window. Se ao invés disso você quer parar um bot em particular enquanto mantém os outros rodando, então dê uma olhada no **[parâmetro de configuração do bot](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Configuration-pt-BR#configura%C3%A7%C3%A3o-do-bot)** `Enabled`, ou no **[comando](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Commands-pt-BR)** `stop`. Caso você queira interromper o processo de farm automático, mas manter o ASF em execução para sua conta, então essa é a finalidade da opção `FarmingPausedByDefault` em `FarmingPreferences` na **[propriedade de configuração do bot](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Configuration-pt-BR#configuração-do-bot)** e do **[comando](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Commands-pt-BR)** `pause`.

---

### Quantos bots posso rodar com o ASF?

Como programa, o ASF não tem nenhum limite máximo de instâncias de bot, então você pode rodar o quanto quiser, enquanto você tiver memória disponível em seu computador, no entanto, você ainda estará limitado pela rede e pelos outros serviços do Steam. Atualmente você pode rodar de 100 a 200 bots por IP e por instância do ASF. É possível rodar mais bots com mais IPs e mais instâncias do ASF, contornando as limitações de IP. Tenha em mente que caso você estiver usando essa grande quantidade de bots, você deve controlar o número deles você mesmo, garantindo que todos eles estejam de fato fazendo login e funcionando ao mesmo tempo. O ASF não foi desenvolvido para essa enorme quantidade de bots, e a regra geral se aplica de que **quanto mais bots você tiver, mais problemas encontrará**. Também observe que o limite acima, em geral, depende de muitos fatores internos, é uma aproximação em vez de um limite estrito - você provavelmente poderá executar mais/menos bots do que o especificado acima.

A equipe do ASF sugere **possuir** até **10 contas Steam no total**, e portanto, também executar até **10 bots no total**. Qualquer coisa acima não é suportada e é feita por sua própria conta e risco, contra nossa sugestão feita aqui. Essa recomendação é feita tanto com base em diretrizes internas da Valve quanto nossas sugestões. Se você vai cumprir essa regra ou não é sua escolha, o ASF como uma ferramenta não irá contra sua própria vontade, mesmo que isso resulte na suspensão de suas contas do Steam por fazê-lo. Portanto, o ASF exibirá um aviso caso você ultrapassar o que recomendamos, mas ainda permitirá que você execute o que quiser por sua própria conta e risco e sem o nosso suporte.
...
### `Não foi possível iniciar a sessão no Steam: TryAnotherCM/Invalid`, `ServiceUnavailable/Invalid`

Conforme mencionado acima, mas desta vez o servidor com o qual você se conectou está explicitamente indisponível. Geralmente ocorre durante o período de manutenção do Steam, não há nada que você possa fazer sobre isso. O ASF tentará automaticamente com um servidor diferente até que um deles aceite sua solicitação. Não deve durar mais do que uma hora no máximo.
...
### `System.InvalidOperationException: Não é possível ler teclas quando o aplicativo não possui um console ou quando a entrada do console foi redirecionada`
### `System.IO.IOException: Input/output error`
### `RequestInput() entrada é inválida!`

Caso esse erro tenha ocorrido durante a entrada no ASF (por exemplo, se você vê `GetUserInput()` na stacktrace), então ele foi causado pelo seu ambiente, que impede o ASF de ler a entrada padrão do seu console. Isso pode ocorrer por diversos motivos, mas o mais comum é você estar executando o ASF no ambiente errado (por exemplo, usando `systemd`, `nohup` ou em segundo plano com `&` em vez de usar `screen` no Linux). Se o ASF não puder acessar sua entrada padrão, então você verá este erro no registro e a incapacidade do ASF de usar seus dados durante o tempo de execução.

Normalmente, você deve corrigir o problema acima, ou seja, permitir que o ASF acesse a entrada padrão para que você possa fornecer os dados necessários. No entanto, caso você **espere** que isso aconteça e **pretenda** executar o ASF em um ambiente sem entrada, então deve informar explicitamente ao ASF que este é o caso, configurando o modo **[`Headless`](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Configuration-pt-BR#headless)** de forma apropriada. Isso dirá ao ASF para nunca pedir entradas do usuário sob nenhuma circunstância, permitindo que você execute o ASF em ambientes sem entradas com segurança. Você pode responder a entradas selecionadas por outros meios nesse modo, por exemplo, pela ASF-ui.
...
Se você tem certeza de que a data no seu computador está certa e mesmo assim o erro persiste, então, assumindo que não é uma questão temporária, os certificados SSL em que seu sistema confia podem estar desatualizados ou inválidos. Neste caso você deve garantir que seu computador pode estabelecer conexões seguras, por exemplo, verificando se você pode acessar `https://github.com` com qualquer navegador de sua escolha, ou uma ferramenta CLI como `curl`. Caso você tenha confirmado que isso funciona corretamente, sinta-se à vontade para nos pedir suporte.
...
**Certifique-se de que você baixou o ASF de uma fonte confiável**. A única fonte oficial e confiável é a página de ****[lançamentos do ASF](https://github.com/JustArchiNET/ArchiSteamFarm/releases/latest)**** no GitHub (que também é a fonte para as atualizações automáticas do ASF). ****Qualquer outra fonte é considerada não confiável por definição e pode conter malware adicionado por terceiros****. Você não deve confiar em nenhum outro local de download; sempre garanta que seu ASF venha de nós.
