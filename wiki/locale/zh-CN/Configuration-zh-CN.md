...
这是一个默认值为 `false` 的 `bool` 类型属性。 该属性定义是否要在 Headless 模式下运行进程。 在 Headless 模式下，ASF 会假定自己运行于服务器或者其他非交互式环境，因此它将不会尝试在控制台输入中读取任何信息。 这包括程序请求的细节（帐户的关键凭据，如两步验证代码、Steam 令牌验证码、密码或者其他 ASF 操作所需的任何变量），以及所有其他控制台输入（例如交互式命令控制台）。 换句话说，`Headless` 模式意味着 ASF 的控制台是只读的。 此设置主要面向在服务器上运行 ASF 的用户，因为在需要向用户询问信息（例如两步验证代码）时，ASF 将会直接停用相关的帐户以中止操作。 除非您在服务器上运行 ASF，并且您已经确认 ASF 能够在非 Headless 模式下正常运行，否则应该禁用此属性。 在 Headless 模式下，任何用户交互都将被拒绝，如果您的帐户需要在启动时输入**任何**信息，则它们将不会启动。 这对于服务器来说很有用，因为 ASF 可以在要求提供凭据时中止帐户登录的尝试，而不是无限地等待用户提供凭据。

启用此模式将允许您在需要向控制台输入时，通过其他方式来提供，包括： ASF-ui（ASF API），或通过 **[`input`](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Commands-zh-CN#input-命令)** 命令。 如果您不确定应该如何设置这个属性，请保留默认值 `false`。
...
这是一个默认值为 `4` 的 `byte` 类型属性。 ASF 会确保连续两个 Web 库存请求之间至少间隔 `InventoryLimiterDelay` 秒，以避免触发频率限制——这主要发生在将库存通知标记为已读时，也可能被第三方插件用于拉取其他用户的库存。 此属性不会用于获取我们自己的库存，因为 ASF 为此使用效率更高的内部网络调用，所以此属性完全不会影响 `loot` 或 `transfer` 这种命令。 我们基于连续标记上百个机器人库存的数据设定了默认值 `4`，这个值应该满足绝大多数用户的需求。 如果您的机器人数量很少，可能希望减小这个值甚至更改为 `0`，使 ASF 忽略延迟，更快地标记 Steam 库存。 但请注意，设置过低的值**将会**导致 Steam 临时封禁您的 IP，彻底阻止您在这段时间内继续发送任何请求。 如果您有大量机器人，并且发送大量请求，则可能还需要增大此值，不过在这种情况下您可能需要考虑设法限制请求的数量。 除非您有**充分的**理由编辑此属性，否则应将其保留为默认值。
...
这是一个默认值为空的 `ImmutableHashSet<string>` 类型属性。 此属性定义一个插件组件名称列表，以作为插件自动更新的黑名单或白名单，具体模式由下文的 `PluginsUpdateMode` 属性指定。
...
这是一个默认值为 `0` 的 `byte` 类型属性。 此属性定义上文 `PluginsUpdateList` 属性所使用的插件更新模式。 通过指定此属性，您可以轻松启用/禁用所有插件的自动更新，但您明确指定的插件除外。

- 值为 `0`，表示`Whitelist`（白名单），**禁用**所有插件的自动更新，在 `PluginsUpdateList` 属性中定义的除外。
- 值为 `1`，表示`Blacklist`（黑名单），**启用**所有插件的自动更新，在 `PluginsUpdateList` 属性中定义的除外。

**ASF 团队提醒您，为了您的安全，您只应该为信任来源启用自动更新。** 请注意，**无论**此处如何设置，恶意插件都可以更新自身或远程执行命令，因此，该设置只适用于 ASF 自带的插件更新功能，并且您仍然应该确保在使用插件之前验证每个插件的安全性。

默认情况下，根据 **[`UpdateChannel`](#updatechannel)** 和 **[`UpdatePeriod`](#updateperiod)**，插件的更新与 ASF 更新操作一起执行。 标准的 ASF 更新机制，例如 `update` 命令也可以触发插件更新。 如果您只想要手动更新插件，而不同时更新 ASF 版本， 则可以使用 `updateplugins` 命令。
...
这是一个默认值为 `false` 的 `bool` 类型属性。 启用时，ASF 会在可能的情况下——也就是当所有已注册的机器人停止时——尝试关闭进程。 此选项的主要使用方式是与所有机器人实例的 `ShutdownOnFarmingFinished` 配置相结合，这样，当最后一个机器人完成挂卡时，ASF 将被允许自动关闭。

因为大多数用户都期望进程始终运行，例如在使用 `IPC` 时，所以此选项默认禁用。
...
默认情况下，ASF 会使用所有可用的 Steam 协议，作为应对 Steam 服务器宕机以及其他类似问题的手段。 如果您需要将 ASF 限制为只使用其中一个或两个特定的协议，则需要更改此属性。 这些手段可能有各种用途，例如，在防火墙上阻止 UDP 流量，并确保只有 TCP 流量通过，或者您设置了 `WebProxy` 并且希望 Steam 客户端连接也使用代理，则可以限制为 `WebSocket`，因为只有这种协议支持代理。
...
这是一个默认值为 `1` 的 `byte` 类型属性。 该属性定义了 ASF 使用的更新通道，自动更新（`UpdatePeriod` 大于 `0` 时）和更新提醒（反之）功能都会用到这个属性。 目前 ASF 支持三个更新通道——`0` 表示 `None`（不更新），`1` 表示 `Stable`（稳定通道），`2` 表示 `PreRelease`（预发布通道）。 `Stable` 是默认的发布通道，适合大多数用户。 `PreRelease` 不仅包含 `Stable`（稳定版）更新，还包括 **pre-releases**（预览版），供高级用户和其他开发者测试新特性、验证修复补丁或者对计划中的功能改进进行反馈。 **预发布通道的版本通常包含未修复的漏洞、未完成的特性或者功能重写**。 如果您认为自己不属于高级用户，请保留该属性的默认值 `1`（稳定更新通道 `Stable`）。 `PreRelease` 通道专门针对了解如何报告漏洞、处理问题、提交反馈的用户——我们不对此提供技术支持。 如果您想了解更多，请阅读[**发布周期**](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Release-cycle-zh-CN)。 如果您希望完全禁用版本检查，也可以将 `UpdateChannel` 设置为 `0`（`None`）。 设置 `UpdateChannel` 为 `0` 将会禁用一切与更新有关的功能，包括 `update` 命令。 **强烈不建议**您使用 `None` 通道，因为这会将您自己暴露在各种类型的问题下（见下文 `UpdatePeriod` 的说明）。
...
这是一个默认值为 `null` 的 `string` 类型属性。 该属性定义了一个 Web 代理地址，内部 HTTP 相关通讯都会经过此代理，特别是其中的 `github.com`、`api.steampowered.com`、`steamcommunity.com` 和 `store.steampowered.com` 服务。 这特指常规（非机器人的）通信，但如果机器人自身的 `WebProxy` 配置属性未设置，也代理机器人的通信。 为 ASF 请求设置代理对于绕过各种形式的防火墙（特别是中国的网络防火墙）非常有用。
...
如果您也需要代理内部的 Steam 网络通讯（CM），则应该将 **[`SteamProtocols`](#steamprotocols)** 机器人属性配置为只允许 WebSocket 传输，即值 `4`，因为只有 WebSocket 支持代理。
...
| 64 | DisableIncomingTradesParsing  | 使 ASF 永不处理收到的交易报价                     |

请注意，该属性是 `flags` 字段，因此可以设置为可用选项的任意组合。 如果您想了解更多，请阅读 **[JSON 映射](#json-映射)**。 不启用任何 Flag 即为 `None` 选项。

一般来说，如果您希望调整各种与机器人动态相关的自动化设置，就需要修改这个属性。 默认的配置使 ASF 对帐户的影响最小，只允许不违背用户意愿的、有益的自动化操作。
...
`DisableIncomingTradesParsing` 会阻止 ASF 解析任何收到的交易报价，这意味着所有与其相关的交易功能都不可用。 因为 ASF 默认在影响最小的模式下工作，只接受权限达到 `Master` 或以上用户的交易报价，并忽略其他交易报价——所以默认会启用交易报价解析。 对于希望不产生额外交易报价请求/开销的用户来说，这样设置很合理，能够完全禁用整个处理逻辑，例如，他们知道机器人永远不会收到 Master 交易报价，也就完全不需要 ASF 参与交易活动。 请注意，指定此选项将禁用所有其他依赖接收交易的选项，例如 `AcceptDonations` 或 `SteamTradeMatcher`——自定义插件也无法以常规方式处理收到的交易报价。
...
`ShutdownOnFarmingFinished` 允许您在挂卡完成后停止机器人。 通常，ASF 会在机器人激活期间始终“占用”帐户。 在指定帐户挂卡完成后，ASF 会定期（每隔 `IdleFarmingPeriod` 小时）检查帐户内是否有新游戏含有 Steam 卡牌，以便于在不重启进程的情况下恢复此帐户的挂卡过程。 这应该适合大多数人，因为 ASF 可以在需要时自动恢复挂卡。 但是，您可能希望在指定帐户挂卡完成后停止进程，这就需要设置此选项。 在启用时，ASF 将会在挂卡完成之后注销帐户，这意味着此帐户将不再被定期检查或占用。 您应该自己决定，更希望 ASF 始终处理指定的机器人实例，还是在挂卡结束后停止。

此选项通常会与 `ShutdownIfPossible` 相结合，所以当所有帐户都停止运行时，ASF 本身也会关闭，让您的机器休息，您可以在获得最后一张卡牌之后为计算机安排睡眠或者关机等其他操作。
...
`AutoUnpackBoosterPacks` 将会在收到新物品通知时自动拆开所有补充包。 这会让您立刻获得额外的卡牌掉落，这可能正是您所需要的，特别是在与其他选项（例如 `SteamTradeMatcher` 或 `CompleteTypesToSend`）组合的情况下。
...
这是一个默认值为 `0` 的 `byte flags` 类型属性。 此选项可以启用一些额外的 Steam 平台在线功能，如下所述：

| 值 | 名称          | 描述               |
| - | ----------- | ---------------- |
| 0 | None        | 无特殊在线偏好，默认值      |
| 1 | IsSteamDeck | 显示自己为 Steam Deck |
...
这是一个默认值为 `0` 的 `byte` 类型属性。 该属性指定机器人在登录到 Steam 网络后将自己广播为哪种用户界面模式。 如果您的 `OnlineStatus` 选项支持，则它可能会影响帐户在 Steam 聊天等处的显示方式。 目前您可以选择下列模式之一：

| 值   | 名称             | 描述             |
| --- | -------------- | -------------- |
| `0` | VGUI           | 默认 Steam 客户端模式 |
| `1` | Tenfoot        | 大屏幕模式          |
| `2` | Mobile         | Steam 手机应用     |
| `3` | Web            | 网页浏览器会话        |
| `4` | ClientUI       |                |
| `5` | MobileChat     | Steam 聊天手机应用   |
| `6` | EmbeddedClient |                |

并非所有模式都会产生效果。 此外，您可能有兴趣了解 `OnlinePreferences`，因为一些额外功能需要在那里启用。 如果您不确定应该如何设置这个属性，请保留默认值 `0`。

---

### `WebProxy`

这是一个默认值为 `null` 的 `string` 类型属性。 该属性定义了一个 Web 代理地址，此机器人内部的 HTTP 相关通讯都会经过此代理，特别是其中的 `api.steampowered.com`、`steamcommunity.com` 和 `store.steampowered.com` 服务。 如果未设置，ASF 将使用上文所述的全局 `WebProxy` 设置。 为 ASF 请求设置代理对于绕过各种形式的防火墙（特别是中国的网络防火墙）非常有用。
