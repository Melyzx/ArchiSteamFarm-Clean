...
* **[Trình tạo cấu hình ](#web-based-configgenerator)**
* **[Cấu hình ASF-ui](#asf-ui-configuration)**
...
**Đảm bảo rằng bạn không chia sẻ `LicenseID` của bạn với những người khác**. Since it's issued on personal basis, it might get revoked if it's leaked. If by any chance this happened to you accidentally, you can generate a new one from the same place.
...
Trừ khi bạn có lý do để chỉnh sửa thuộc tính này, bạn nên để nó theo giá trị mặc định.
...
**Trừ khi bạn biết bạn đang làm gì**, chúng tôi **cực kì** khuyên bạn để nó theo giá trị mặc định.
...
Cấu hình bot có cấu trúc sau đây:
...
| Giá trị | Tên                           | Mô tả                                                                                                    |
| ------- | ----------------------------- | -------------------------------------------------------------------------------------------------------- |
| 0       | Trống                         | No special bot behaviour, sane default settings                                                          |
...
| 4       | DismissInventoryNotifications | Will cause ASF to automatically dismiss all inventory notifications                                      |
...
Nếu bạn không rõ cách chỉnh tùy chọn này, tốt nhất nên để nó theo giá trị mặc định.
...
| Giá trị | Tên                       | Mô tả                                                                                       |
| ------- | ------------------------- | ------------------------------------------------------------------------------------------- |
| 0       | Unordered                 | Không sắp xếp gì, cải thiện hiệu suất CPU một chút                                          |
| 1       | AppIDsAscending           | Cố cày trò chơi với giá trị `appIDs` thấp nhất trước                                        |
| 2       | AppIDsDescending          | Cố cày trò chơi với giá trị `appIDs` cao nhất trước                                         |
| 3       | CardDropsAscending        | Cố cày trò chơi với số lần rớt thẻ thấp nhất trước                                          |
| 4       | CardDropsDescending       | Cố cày trò chơi với số lần rớt thẻ cao nhất trước                                           |
| 5       | HoursAscending            | Cố cày trò chơi với số giờ chơi thấp nhất trước                                             |
| 6       | HoursDescending           | Cố cày trò chơi với số giờ chơi cao nhất trước                                              |
| 7       | NamesAscending            | Cố cày trò chơi theo thứ tự bảng chữ cái, bắt đầu từ A                                      |
| 8       | NamesDescending           | Cố cày trò chơi theo thứ tự bảng chữ cái ngược, bắt đầu từ Z                                |
| 9       | Random                    | Cố cày trò chơi theo thứ tự hoàn toàn ngẫu nhiên (khác biệt theo mỗi lần chạy của phần mềm) |
| 10      | BadgeLevelsAscending      | Cố cày trò chơi với cấp độ huy hiệu thấp nhất trước                                         |
| 11      | BadgeLevelsDescending     | Cố cày trò chơi với cấp độ huy hiệu cao nhất trước                                          |
| 12      | RedeemDateTimesAscending  | Cố cày trò chơi cũ nhất trên tài khoản của chúng ta trước                                   |
| 13      | RedeemDateTimesDescending | Cố cày trò chơi mới nhất trên tài khoản của chúng ta trước                                  |
| 14      | MarketableAscending       | Cố cày trò chơi với với thẻ rớt không thể bán được trước                                    |
| 15      | MarketableDescending      | Cố cày trò chơi với với thẻ rớt có thể bán được trước                                       |
...
Mọi tùy chọn đã được mô tả ở dưới.
...
Trừ khi bạn biết bạn đang làm gì, bạn nên để nó theo giá trị mặc định là `5`.
...
Nếu bạn không chắc về cách đặt thuộc tính này, hãy để nó với giá trị mặc định là `0`.
...
| Value | Tên         | Mô tả                                  |
| ----- | ----------- | -------------------------------------- |
| 0     | Trống       | No special online preferences, default |
...
Nếu bạn không chắc về cách đặt thuộc tính này, chúng tôi đề xuất đặt giá trị `0` (`Rời mạng`) cho tài khoản chính, và nếu không thì đặt mặc định là `1` (`Trên mạng`).
...
| Giá trị | Tên                                | Mô tả                                                                                                                           |
| ------- | ---------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| 0       | None (Không có)                    | No special redeeming preferences, default                                                                                       |
| 1       | Forwarding                         | Forward keys unavailable to redeem to other bots                                                                                |
| 2       | Distributing                       | Phân phối tất cả mã kích hoạt với chính nó và các bot khác                                                                      |
...
| 1       | SteamGroup (Nhóm Steam) | Cho phép giao tiếp với **[nhóm Steam của ASF](https://steamcommunity.com/groups/archiasf)**                                                                                                                                                                       |
...
Further explanation on this subject is available in **[remote communication](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Remote-communication)** section. Trừ khi bạn có lý do để chỉnh sửa thuộc tính này, bạn nên để nó theo giá trị mặc định.
...
Not all modes might have an effect. Also, you might be interested in checking `OnlinePreferences`, since some additional features are enabled there. Nếu bạn không chắc về cách đặt thuộc tính này, hãy để nó với giá trị mặc định là `0`.
...
Ví dụ: `"SteamMasterClanID": 103582791440160998`
...
Ví dụ: `"SteamLogin": null`, `"SteamLogin": ""`, `"SteamLogin": "MyAccountName"`
...
Ví dụ: `"LicenseID": null`, `"LicenseID": "f6a0529813f74d119982eb4fe43a9a24"`
...
Ví dụ ccho `ImmutableList<byte>`: `"FarmingOrders": [15, 11, 7]`
...
Ví dụ cho `ImmutableHashSet<uint>`: `"Blacklist": [267420, 303700, 335590]`
...
Ví dụ cho `ImmutableDictionary<ulong, byte>`: `"SteamUserPermissions": { "76561198174813138": 3, "76561198174813137": 1 }`
