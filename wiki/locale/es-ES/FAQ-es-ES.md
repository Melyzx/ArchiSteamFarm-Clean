...
- **Tu juego no puede estar marcado como [privado](https://help.steampowered.com/es/faqs/view/1150-C06F-4D62-4966), ASF omitirá automáticamente dichos juegos durante la recolección.**
...
La mejor forma de lograrlo es usando la propiedad de configuración **[`GamesPlayedWhileIdle`](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Configuration-es-ES#gamesplayedwhileidle)**, lo que hará que se jueguen las appIDs seleccionadas cuando ASF no tenga cromos para recolectar. Si quieres jugar tus juegos todo el tiempo, incluso si tienes cromos obtenibles de otros juegos, puedes combinarlo con **[`FarmPriorityQueueOnly`](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Configuration-es-ES#farmingpreferences)**, así ASF solo recolectará cromos de aquellos juegos que explícitamente establezcas, o alternativamente **[`FarmingPausedByDefault`](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Configuration-es-ES#farmingpreferences)**, que mantendrá pausado el módulo de recolección cromos hasta que lo reanudes manualmente.

También puedes usar el comando **[`play`](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Commands-es-ES#comandos-1)**, que hará que ASF juegue tus juegos seleccionados. Sin embargo, ten en cuenta que `play` solo debe utilizarse para juegos que quieras jugar temporalmente, ya que no es un estado persistente, causando que ASF regrese al estado predeterminado, por ejemplo, tras una desconexión de la red de Steam. Por lo tanto, recomendamos que uses `GamesPlayedWhileIdle`, a menos que realmente quieras iniciar tus juegos seleccionados por un corto período de tiempo, y luego volver al flujo normal.
...
Por lo tanto, como con cada software de Steam, ASF no está autorizado por Valve y no puedo garantizar que no serás suspendido si Valve de repente decide banear las cuentas que usen ASF. Esto es extremadamente improbable considerando el hecho de que ASF es usado en varios millones de cuentas, pero sigue siendo una posibilidad, a pesar de la probabilidad real.
...
Este caso es, una vez más, muy difícil de analizar, por la respuesta vaga del soporte de Steam que apenas ofrece detalles. Basado en mi opinión personal, este usuario probablemente intercambió cromos de Steam por algún tipo de dinero (¿bot para subir de nivel?) o de alguna otra forma intentó retirar fondos de Steam. En caso de que no lo sepas, esto también es ilegal de acuerdo a los **[Términos de Servicio de Steam](https://store.steampowered.com/subscriber_agreement/spanish/)**. No estamos en posición de decirte lo que puedes hacer con los cromos obtenidos mediante ASF - pero el usuario en cuestión definitivamente no solo fabricó insignias con ellos.
...
> Hola XXX, Gracias por contactar con el Soporte de Steam. Esta y otras cuentas fueron usadas para inundar nuestra infraestructura de red, lo cual es una violación de las normas de conducta online de Steam. Esta cuenta ha sido bloqueada permanentemente y el Soporte de Steam no puede proporcionar soporte adicional con este problema.
...
Todos lo incidentes anteriores tienen algo en común - ASF solo es una herramienta y es **tu** decisión cómo vas a usarlo. No serás baneado solo por usar ASF, sino por **cómo** lo usas. Puede ser una herramienta útil para recolectar una sola cuenta, o una red masiva de recolección formada por miles de bots. En cualquiera caso, no estoy ofreciendo asesoría legal, y tú debes decidir cómo usar ASF. No estoy ocultando ninguna información que podría ayudarte, por ejemplo, el hecho de que ASF causó que banearan a algunas personas (y en qué contexto), pues no tengo ninguna razón para hacerlo - es tu decisión lo que quieras hacer con esa información.
...
Simplemente cierra el proceso de ASF, por ejemplo, haciendo clic en [X] en Windows. Si en cambio quieres detener un bot en particular pero mantener los demás en ejecución, entonces echa un vistazo a la **[propiedad de configuración del bot](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Configuration-es-ES#configuraci%C3%B3n-de-bot)** `Enabled`, o al **[comando](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Commands-es-ES)** `stop`. Si en cambio quieres detener el proceso de recolección automática, pero mantener ASF ejecutándose para tu cuenta, entonces para eso son la opción `FarmingPausedByDefault` de la **[propiedad de configuración del bot](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Configuration-es-ES#configuración-de-bot)** y el **[comando](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Commands-es-ES)** `pause`.
...
### `No se puede iniciar sesión en Steam: TryAnotherCM/Invalid`, `ServiceUnavailable/Invalid`
...
### No puedo introducir los datos de acceso que ASF está solicitando
...
Si este error ocurrió durante la entrada de ASF (por ejemplo, puedes ver `GetUserInput()` en el stacktrace) entonces es causado por tu entorno el cual impide que ASF lea la entrada estándar de tu consola. Esto puede ocurrir debido a muchas razones, pero la más común es que ejecutes ASF en el entorno incorrecto (por ejemplo, en `systemd`, `nohup` o `&` en segundo plano en lugar de `screen` en Linux). Si ASF no puede acceder a su entrada estándar, entonces verás este error registrado y la incapacidad de ASF de usar tus datos durante el tiempo de ejecución.

Normalmente deberías corregir el problema anterior, es decir, permitir que ASF acceda la entrada estándar, para que puedas ingresar los detalles. Sin embargo, si  **esperas** que esto ocurra, es decir, que **pretendes** ejecutar ASF en un entorno sin entrada, entonces deberías informarle explícitamente a ASF que ese es el caso, configurando correctamente el modo **[`Headless`](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Configuration-es-ES#headless)** Esto le indicará a ASF que nunca solicite interacción del usuario bajo ninguna circunstancia, permitiéndote ejecutar ASF en entornos sin entrada de forma segura. Puedes responder las peticiones de entrada a través de otros medios en este modo, por ejemplo, en ASF-ui.
...
**Asegúrate de que descargaste ASF de una fuente confiable**. La única fuente oficial y confiable es la página de **[versiones de ASF](https://github.com/JustArchiNET/ArchiSteamFarm/releases/latest)** en GitHub (y esta es también la fuente de las actualizaciones automáticas de ASF) - **inherentemente cualquier otra fuente no es de confianza y puede contener malware añadido por otras persoans** - no debes confiar en ninguna otra fuente de descarga, asegúrate de que tu ASF siempre provenga de nosotros.
