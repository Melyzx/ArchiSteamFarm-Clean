...
Habilitar este modo te permitirá proporcionar las entradas de consola requeridas a través de otros medios, por ejemplo,  ASF-ui (ASF API), o mediante el comando **[`input`](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Commands-es-ES#comando-input)**. Si no estás seguro de cómo establecer esta propiedad, déjala con su valor predeterminado de `false`.
...
Tipo `byte` con valor predeterminado de `4`. ASF se asegurará de que haya al menos `InventoryLimiterDelay` segundos entre dos solicitudes del inventario web consecutivas para evitar que se alcance el límite de solicitudes - estas son utilizadas para marcar las notificaciones del inventario como leídas, también podrían ser usadas por plugins de terceros para analizar el inventario de otros usuarios. Esta propiedad no se usa para obtener nuestro propio inventario, ya que ASF usa una llamada de red interna mucho más eficiente para eso, así que no afectará a comandos como `loot` o `transfer`. El valor predeterminado de `4` fue establecido basado en marcar el inventario de más de 100 instancias de bot consecutivas, y debería satisfacer a la mayoría (si no a todos) de usuarios. Sin embargo tal vez quieras reducirlo, o incluso cambiarlo a `0` si tienes una cantidad pequeña de bots, así ASF ignorará el retraso y marcará los inventarios de Steam mucho más rápido. Sin embargo, debes tener cuidado, ya que establecer un valor muy bajo **resultará** en que Steam bloquee temporalmente tu IP, y te impedirá hacer solicitudes. También podría ser necesario que incrementes este valor si estás ejecutando muchos bots con muchas solicitudes de inventario, aunque en este caso tal vez debas tratar de limitar el número de esas solicitudes. A menos que tengas una **buena** razón para editar esta propiedad, deberías dejarla en su valor predeterminado.
...
Tipo `ImmutableHashSet<string>` con valor predeterminado estando vacío. Esta propiedad define la lista de plugins que están ya sea bloqueados o permitidos para ser considerados para actualizaciones automáticas, de acuerdo a `PluginsUpdateMode` definido a continuación.
...
Tipo `byte` con valor predeterminado de `0`. Esta propiedad define el modo de actualización de plugins que da significado a `PluginsUpdateList` definido arriba. Al especificar esta propiedad puedes habilitar/deshabilitar las actualizaciones automáticas para todos los plugins excepto los declarados.

- Valor de `0`, llamado `Whitelist` (lista blanca), **deshabilita** las actualizaciones automáticas para todos los plugins, excepto aquellos definidos en `PluginsUpdateList`.
- Valor de `1`, llamado `Blacklist` (lista negra), **habilita** las actualizaciones automáticas para todos los plugins, excepto aquellos definidos en `PluginsUpdateList`.

**El equipo de ASF desea recordarte que, por tu propia seguridad, solo deberías habilitar las actualizaciones automáticas de fuentes confiables**. Ten en cuenta que los plugins maliciosos pueden decidir actualizarse o ejecutar comandos remotos **a pesar** de esta configuración, debido a que esta configuración aplica exclusivamente para la función de actualizaciones de los plugins proporcionados por ASF, y aún deberías asegurarte de verificar cada plugin que decidas usar.

Las actualizaciones de plugins se realizan por defecto junto con la rutina de actualización de ASF - **[`UpdateChannel`](#updatechannel)** y **[`UpdatePeriod`](#updateperiod)**. Los mecanismos de actualización estándar, como el comando `update`, también activarán la actualización de los plugins opcionales. Si en cambio deseas actualizar los plugins manualmente sin actualizar la versión de ASF, el comando `updateplugins` ofrece dicha posibilidad.
...
Tipo `bool` con valor predeterminado de `false`. Cuando esta opción se encuentra habilitada, ASF intentará terminar el proceso si es posible, es decir, cuando todos tus bots registrados se detengan. Esto puede ser especialmente útil cuando se combina con `ShutdownOnFarmingFinished` en todas tus instancias de bot, ya que así ASF tendrá permitido cerrarse automáticamente cuando el último de tus bots termine de recolectar.

Ya que la expectativa para la mayoría de usuarios es tener el proceso en ejecución en todo momento, por ejemplo, para usar `IPC`, esta opción está deshabilitada por defecto.
...
Por defecto ASF utilizará todos los protocolos de Steam disponibles como una medida para evitar caídas y otros problemas similares de Steam. Normalmente querrás cambiar esta propiedad si deseas limitar ASF para usar solamente uno o dos protocolos específicos. Estas medidas podrían ser necesarias en varias situaciones, por ejemplo, si has bloqueado el tráfico UDP en tu firewall y quieres asegurar que solamente funcione el tráfico TCP, o si estás usando `WebProxy` y quieres usarlo también para la conexión del cliente de Steam, ya que solo el protocolo `WebSocket` soporta eso.
...
Tipo `byte` con valor predeterminado de `1`. Esta propiedad define el canal de actualización que será usado, ya sea para actualizaciones automáticas (si `UpdatePeriod` es mayor a `0`), o (en caso contrario) notificaciones de actualización. Actualmente ASF soporta tres canales de actualización - `0` llamado `None`, `1`, llamado `Stable`, y `2`, que se llama `PreRelease`. El canal `Stable` es el canal de actualización predeterminado, que debería ser utilizado por la mayoría de los usuarios. El canal `PreRelease` además de las versiones `Stable`, también incluye **prelanzamientos** dedicados a usuarios avanzados y otros desarrolladores para probar nuevas características, confirmar correcciones o proporcionar comentarios sobre mejoras planificadas. **Las versiones de prelanzamiento a menudo contienen bugs sin corregir, funciones aún en desarrollo o implementaciones reescritas**. Si no te consideras un usuario avanzado, mantén el canal de actualizaciones predeterminado `1` (`Stable`). El canal `PreRelease` está dedicado a usuarios que saben cómo reportar bugs, lidiar con problemas y dar retroalimentación - no se dará soporte técnico. Echa un vistazo al **[ciclo de lanzamiento](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Release-cycle-es-ES)** de ASF si quieres saber más. También puedes establecer `UpdateChannel` a `0` (`None`), si quieres remover por completo la comprobación de versiones. Establecer `UpdateChannel` a `0` deshabilitará toda funcionalidad relacionada con actualizaciones, incluyendo el comando `update`. **No se recomienda** usar el canal `None` debido a que te expones a toda clase de problemas (mencionado en `UpdatePeriod` a continuación).
...
Tipo `string` con valor predeterminado de `null`. Esta propiedad define una dirección proxy web que será usada para la comunicación interna relativa al http, especialmente a servicios tales como `github.com`, `api.steampowered.com`, `steamcommunity.com` y `store.steampowered.com`. Esto aplica a la comunicación general (no específica de bot), así como a la comunicación específica de bot si su propiedad de configuración equivalente `WebProxy` no está configurada. Hacer proxy a las solicitudes de ASF puede ser especialmente útil para evadir varios tipos de cortafuegos, especialmente el gran cortafuegos en China.
...
Si también requieres hacer proxy a la comunicación interna de la red de Steam (CMs), entonces debes asegurarte de configurar la propiedad del bot **[`SteamProtocols`](#steamprotocols)** a un valor que permita solo websocket, como a un valor de `4`, ya que solo los websockets están soportados para hacer proxy.
...
| Valor | Nombre                        | Descripción                                                                                                               |
| ----- | ----------------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| 0     | None                          | Sin comportamiento especial, configuración por defecto adecuada.                                                          |
...
| 64    | DisableIncomingTradesParsing  | Causará que ASF nunca procese ofertas de intercambio entrantes.                                                           |
...
En general, querrás modificar esta propiedad si esperas cambiar alguna de las configuraciones de automatización relacionadas con la actividad del bot. La configuración predeterminada implica que ASF se ejecute en modo no invasivo, habilitando únicamente una automatización beneficiosa que no va en contra de la voluntad del usuario.
...
`DisableIncomingTradesParsing` evitará que ASF analice ofertas de intercambio entrantes, esto significa que toda la funcionalidad de intercambio relacionada no estará operativa. Puesto que ASF funciona por defecto en el modo menos invasivo, solo aceptará ofertas de intercambio de usuarios `Master` o superior, nunca tocando otras ofertas de intercambio - el análisis de las ofertas de intercambio está habilitado por defecto. Esta configuración tiene más sentido para las personas que quieren asegurarse de que no haya solicitudes/sobrecarga adicional relacionada con el análisis de los intercambios, deshabilitando toda la lógica en el proceso, por ejemplo, porque saben que sus bots nunca recibirán solicitudes de intercambio del usuario master, y por lo tanto no requiere que ASF participe en su actividad de intercambio para nada. Ten cuenta que especificar esta opción también deshabilitará todas las demás opciones que dependan de intercambios entrantes, tal como `AcceptDonations` o `SteamTradeMatcher` - los plugins personalizados tampoco serán capaces de procesar las ofertas de intercambio entrantes de manera normal.
...
`AutoUnpackBoosterPacks` abrirá automáticamente todos los packs de refuerzo al recibir un notificación de nuevos artículos. Esto te permitirá adquirir cromos adicionales inmediatamente, lo que podría ser un escenario deseado especialmente en combinación con otras opciones (por ejemplo, `SteamTradeMatcher` o `CompleteTypesToSend`).
...
Tipo `byte flags` con valor predeterminado de `0`. Esta propiedad puede habilitar algunas características en línea adicionales en la plataforma de Steam, y se define a continuación:

| Valor | Nombre      | Descripción                                       |
| ----- | ----------- | ------------------------------------------------- |
| 0     | None        | Sin preferencias en línea especiales, por defecto |
| 1     | IsSteamDeck | Se presenta como Steam Deck                       |
...
Tipo `byte` con valor predeterminado de `0`. Esta propiedad especifica el modo de interfaz de usuario con el cual el bot será anunciado después de iniciar sesión en la red de Steam. Podría influir en cómo la cuenta es visible, por ejemplo, en el chat de Steam, si tu presencia lo permite a través de `OnlineStatus`. Actualmente puedes elegir uno de los siguientes modos:

| Valor | Nombre         | Descripción                        |
| ----- | -------------- | ---------------------------------- |
| `0`   | VGUI           | Modo cliente de Steam por defecto  |
| `1`   | Tenfoot        | Modo Big Picture                   |
| `2`   | Mobile         | Aplicación móvil de Steam          |
| `3`   | Web            | Sesión en navegador web            |
| `4`   | ClientUI       |                                    |
| `5`   | MobileChat     | Aplicación móvil del chat de Steam |
| `6`   | EmbeddedClient |                                    |

No todos los modos podrían tener un efecto. Además, podría interesarte revisar `OnlinePreferences`, ya que algunas características adicionales se habilitan ahí. Si no estás seguro de cómo establecer esta propiedad, déjala con su valor predeterminado de `0`.

---

### `WebProxy`

Tipo `string` con valor predeterminado de `null`. Esta propiedad define una dirección proxy web que se utilizará para la comunicación interna específica del bot, especialmente para servicios como `api.steampowered.com`, `steamcommunity.com` y `store.steampowered.com`. Si no se establece, en su lugar ASF usará la configuración global `WebProxy` especificada anteriormente. Hacer proxy a las solicitudes de ASF puede ser especialmente útil para evadir varios tipos de cortafuegos, especialmente el gran cortafuegos en China.
