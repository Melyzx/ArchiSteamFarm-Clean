...
Auf diese Weise haben wir sichergestellt, dass alles, was in `SHA512SUMS` geschrieben wurde, mit den resultierenden Dateien übereinstimmt und diese nicht manipuliert wurden. Es ist jedoch noch nicht bewiesen, dass die `SHA512SUMS` Datei wirklich von uns kommt. There are two ways to verify that.
...
Wir können das `gpg`-Dienstprogramm für diesen Zweck verwenden, sowohl auf **[Linux](https://gnupg.org/download/index.html)** und **[Windows](https://gpg4win.org)** (ändern Sie den Befehl `gpg` in `gpg.exe` unter Windows).
