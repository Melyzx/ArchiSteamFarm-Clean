...
물론 대상으로 하는 OS 아키텍쳐에 따라 `linux-x64`를 `win-x64` 등으로 변경하십시오. 이 빌드도 업데이트가 비활성화됩니다. When building `--self-contained` you can also optionally declare two more switches: `-p:PublishTrimmed=true` will produce trimmed build, while `-p:PublishSingleFile=true` will produce a single file. Adding both will result in the same settings we use for our own builds.
