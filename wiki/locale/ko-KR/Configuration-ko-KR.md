...
`ImmutableHashSet<string>` 타입으로 기본값은 비어있습니다. This property defines list of plugin assembly names that are either blacklisted or whitelisted for being considered for automatic updates, as per `PluginsUpdateMode` defined below.
...
`byte` 타입으로 기본값은 `0`입니다. This property defines plugins update mode that gives meaning to `PluginsUpdateList` defined above. By specifying this property you can easily enable/disable automatic updates for all plugins except those declared.
...
| 값  | 이름                                          | 설명                                                                                                       |
| -- | ------------------------------------------- | -------------------------------------------------------------------------------------------------------- |
| 0  | 없음(None)                                    | No special bot behaviour, sane default settings                                                          |
...
`byte flags` 타입으로 기본값은 `0`입니다. This property defines ASF behaviour related to farming, and is defined as below:
...
`ShutdownOnFarmingFinished` allows you to shutdown bot once it's done farming. Normally ASF is "occupying" an account for the whole time of process being active. 해당 계정의 농사가 끝났다면, ASF는 주기적으로 매 `IdleFarmingPeriod` 시간마다 Steam 카드가 있는 새로운 게임이 그 사이에 추가되었는지를 확인하여 프로세스를 재시작할 필요없이 농사를 계속할 수 있도록 합니다. 이는 대부분의 사람들에게 유용한데, ASF는 필요하면 자동으로 농사를 이어서할 수 있기 때문입니다. However, you may actually want to stop the process when given account is fully farmed, you can achieve that by using this flag. 활성화되면 ASF는 계정의 농사가 완전히 끝나면 로그오프하여 더이상 주기적으로 체크하거나 점유하지 않게 합니다. ASF가 모든 시간을 봇 인스턴스에 사용하도록 하거나, 혹은 농사 프로세스가 끝나면 멈추게 할지를 스스로 정해야 합니다.
...
`byte flags` 타입으로 기본값은 `0`입니다. This property can enable some additional online features on the Steam platform, and is defined as below:

| 값 | 이름          | 설명                                     |
| - | ----------- | -------------------------------------- |
| 0 | 없음(None)    | No special online preferences, default |
...
`byte` 타입으로 기본값은 `0`입니다. This property works very similar to `SendOnFarmingFinished` preference in `FarmingPreferences`, with one difference - instead of sending trade when farming is done, we can also send it every `SendTradePeriod` hours, regardless of how much we have to farm left. 부계정의 농사가 끝날때까지 기다리는 대신 평소에 `루팅` 하고 싶은 경우 유용합니다. 기본값인 `0`은 이 기능을 비활성화합니다. 예를 들어 봇이 매일 거래를 보내길 원한다면 여기에 `24`를 넣으십시오.
...
`byte` 타입으로 기본값은 `0`입니다. This property specifies user interface mode that the bot will be announced with after logging in to the Steam network. It might influence how the account is visible e.g. on the Steam chat, if your presence allows that through `OnlineStatus`. Currently you can choose one of below modes:
...
Not all modes might have an effect. Also, you might be interested in checking `OnlinePreferences`, since some additional features are enabled there. 이 속성값을 어떻게 설정해야 할지 모르겠다면, 기본값인 `0`으로 두십시오.
