...
Чтобы использовать ASF-ui, у вас должен быть включен интерфейс **[IPC](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/IPC-ru-RU)**. `IPC` включен по умолчанию, поэтому вы можете получить к нему доступ сразу, если вы не отключили его самостоятельно.
...
Enabling this mode will allow you to supply required console input through other means, i.e. ASF-ui (ASF API), or through **[`input`](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Commands#input-command)** command. Если вы не уверены как настроить этот параметр - оставьте ему значение по-умолчанию `false`.
...
Обратите внимание, что независимо от настроек выше, ASF будет отправлять только **[предметы сообщества Steam](https://steamcommunity.com/my/inventory/#753_6)** (`appID` 753, `contextID` 6), поэтому все игровые предметы, подарки, и так далее, исключены из предложений обмена по определению.
...
Обычно вам нужно использовать **[ASF 2FA](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Two-factor-authentication-ru-RU)** вместе с этой функцией, хотя это не обязательно, если вы планируете своевременно обрабатывать подтверждения 2FA вручную. Если вы не уверены как настроить этот параметр - оставьте ему значение по-умолчанию `​0​`.
