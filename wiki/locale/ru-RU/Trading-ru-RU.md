...
ASF включает в себя поддержку не-интерактивных (офлайн) обменов Steam. Как получение (принятие/отклонение), так и отправка обменов работают не требуя дополнительной настройки, однако, требуют аккаунт Steam без ограничений (такой, на котором потрачено больше чем 5$ суммарно). Для аккаунтов с ограничениями доступны только ограниченные торговые функции.
